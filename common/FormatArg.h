...
#ifndef __FORMATARG_H__
#define __FORMATARG_H__
...
namespace detail
{
    ...
    {
        _itoa_s(i, buf, _MAX_U64TOSTR_BASE2_COUNT, 10);
        buf += strlen(buf);
    }

    inline void FormatArg(char*& buf, double i)
    {
        auto r = sprintf_s(buf, _MAX_U64TOSTR_BASE2_COUNT, "%.6f", i);
        buf += r;
    }

    inline void FormatArg(char*& buf, int64_t i)
    {
        auto r = sprintf_s(buf, _MAX_U64TOSTR_BASE2_COUNT, "%lld", i);
        buf += r;
    }

    inline void FormatArg(char*& buf, uint64_t i)
    {
        auto r = sprintf_s(buf, _MAX_U64TOSTR_BASE2_COUNT, "%I64u", i);
        buf += r;
    }
